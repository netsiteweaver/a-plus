<template>
    <RouterLink
        ...
        class="group flex h-full flex-col overflow-hidden rounded-3xl border border-slate-200 bg-white transition hover:border-sky-400/60 hover:shadow-[0_12px_30px_-12px_rgba(56,189,248,0.45)]"
    >
        <div class="relative pb-[70%]">
            ...
            <span
                v-if="product.badge"
                class="absolute left-4 top-4 rounded-full border border-sky-200 bg-sky-100 px-3 py-1 text-xs font-semibold uppercase tracking-[0.28em] text-sky-600"
            >
                {{ product.badge }}
            </span>
        </div>
        ...
        <div class="flex flex-1 flex-col gap-3 p-5">
            <div class="flex items-start justify-between gap-4 text-sm text-slate-500">
                <p class="text-xs uppercase tracking-[0.25em] text-sky-600">in stock</p>
